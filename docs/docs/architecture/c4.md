...
The highest level view showing how the DevOps Reference Platform fits into the broader ecosystem.

```mermaid
C4Context
    title System Context - DevOps Reference Platform

    Person(user, "Platform User", "Developer or operator managing applications")
    Person(enduser, "End User", "User interacting via Slack")

    System(platform, "DevOps Reference Platform", "Kubernetes-based platform demonstrating GitOps, observability, and ChatOps patterns")
...
```mermaid
C4Container
    title Container Diagram - DevOps Reference Platform

    Person(user, "End User", "Manages todos via Slack")
    Person(operator, "Operator", "Manages platform")

    System_Boundary(platform, "DevOps Reference Platform") {
