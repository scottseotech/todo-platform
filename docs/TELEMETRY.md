...
- **Automatic Instrumentation**:
  ...
  - Database queries via GORM (context-aware)
- **Custom Spans**: CreateTodo handler includes custom attributes

### todo-mcp

- **Service Name**: `todo-mcp`
- **Automatic Instrumentation**:
  - HTTP requests via `otelgin` middleware
  - SSE connections tracked
- **MCP Protocol**: Traces include MCP tool calls and resource access
...
SERVICE_VERSION=1.0.0         # Override default version
ENVIRONMENT=production        # Environment name (development, staging, production)

# OTLP Exporter (for Tempo, Jaeger, etc.)
OTEL_EXPORTER_OTLP_ENDPOINT=tempo:4318  # OTLP HTTP endpoint (without http://)

# Stdout Exporter (for local development)
OTEL_ENABLE_STDOUT=true       # Enable pretty-printed trace output to stdout
```
...
- **Grafana Tempo**: Time-series distributed tracing backend
...
The endpoint should be specified **without** the `http://` prefix. The SDK automatically uses HTTP.
...
Traces are pretty-printed to stdout with full span details.

## Local Development

### Option 1: Stdout Only

```bash
# In todo-api directory
export OTEL_ENABLE_STDOUT=true
go run main.go
```

### Option 2: With Grafana Tempo
...
Tempo is deployed as part of the observability stack:

```yaml
# deploy/infrastructure/tempo/values.yaml
tempo:
  receivers:
    otlp:
      protocols:
...
Add environment variables to deployment manifests:

```yaml
# deploy/applications/todo-api/deployment.yaml
env:
...
### Custom Attributes (todo-api)

CreateTodo handler adds:

- `todo.title`: Title of the created todo
- `todo.id`: ID of the created todo
- `todo.created_at`: Creation timestamp
...
1. Access Jaeger: `kubectl port-forward -n observability svc/jaeger-query 16686:16686`
...
### Example: todo-api → todo-mcp

When todo-api calls todo-mcp via HTTP:

1. todo-api creates a span for the outgoing request
2. Trace context injected into HTTP headers
3. todo-mcp extracts context and creates child span
4. Full request path visible in trace visualization
